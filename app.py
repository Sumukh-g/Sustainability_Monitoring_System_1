...
st.set_page_config(
    page_title="Command Center — EcoNexus AI",
    page_icon="🌿",
    layout="wide",
)
...
# ── Command bar ─────────────────────────────────────────────────────
cb1, cb2, cb3, cb4 = st.columns([3, 2, 2, 2])
with cb1:
    st.markdown(
        f"""<div style="padding:0.3rem 0">
            <div style="font-size:1.6rem;font-weight:800;color:{TEXT};letter-spacing:-0.01em">Command Center</div>
            <div style="font-size:0.82rem;color:{TEXT_MUTED}">{PRODUCT_SUBTITLE} for AI Data Centres</div>
        </div>""",
        unsafe_allow_html=True,
    )
with cb2:
    site_str = " / ".join(sites)
    st.markdown(
        f"""<div class="eco-card" style="padding:0.6rem 0.8rem">
            <div class="eco-label">Active Sites</div>
            <div class="eco-value-sm">{site_str}</div>
        </div>""",
        unsafe_allow_html=True,
    )
with cb3:
    ...
    st.markdown(
        f"""<div class="eco-card" style="padding:0.6rem 0.8rem">
            <div class="eco-label">Last Telemetry</div>
            <div class="eco-value-sm">{ts_str}</div>
        </div>""",
        unsafe_allow_html=True,
    )
with cb4:
    ...
    st.markdown(
        f"""<div class="eco-card" style="padding:0.6rem 0.8rem;border-color:{'rgba(0,212,170,0.2)' if n_alerts == 0 else 'rgba(255,107,53,0.2)'}">
            <div class="eco-label">System Status</div>
            <div class="eco-value-sm" style="color:{alert_color}">{health_text}</div>
            <div style="color:{TEXT_MUTED};font-size:0.72rem">{n_alerts} active alert{"s" if n_alerts != 1 else ""}</div>
        </div>""",
        unsafe_allow_html=True,
    )
...
with g1:
    st.markdown(
        f"""<div class="eco-card-highlight" style="text-align:center;padding:1rem">
            <div class="eco-label">Overall Sustainability Performance</div>
        </div>""",
        unsafe_allow_html=True,
    )
...
with dt_col:
    ...
    st.markdown(
        f"""<div style="display:grid;grid-template-columns:1fr;gap:0.2rem;max-width:400px;margin:0 auto">
            <div class="eco-twin-node"><h4>IT Workload</h4><div class="eco-twin-val">{util:.0f}%</div></div>
            <div class="eco-twin-arrow">▼</div>
            <div class="eco-twin-node"><h4>IT Energy</h4><div class="eco-twin-val">{it_energy:.0f} kWh</div></div>
            <div class="eco-twin-arrow">▼</div>
            <div class="eco-twin-node"><h4>Cooling System</h4><div class="eco-twin-val">{cooling:.0f} kW</div></div>
            <div class="eco-twin-arrow" style="display:flex;justify-content:center;gap:3rem">
                <div>▼<br><span style="font-size:0.7rem;color:{CYAN}">Water {water:.0f} L</span></div>
                <div>▼<br><span style="font-size:0.7rem;color:{ORANGE}">Energy</span></div>
            </div>
            <div class="eco-twin-node"><h4>Facility Footprint</h4>
                <div style="display:flex;justify-content:center;gap:1.5rem">
                    <div><span style="color:{TEXT_MUTED};font-size:0.65rem">Energy</span><br><span class="eco-twin-val" style="font-size:1rem">{total_energy:.0f} kWh</span></div>
                    <div><span style="color:{TEXT_MUTED};font-size:0.65rem">Carbon</span><br><span class="eco-twin-val" style="font-size:1rem">{carbon:.1f} kg</span></div>
                </div>
            </div>
        </div>""",
        unsafe_allow_html=True,
...
if alerts:
    ...
    st.markdown(
        f"""<div class="eco-card" style="text-align:center;padding:1rem">
            <span style="color:{GREEN};font-weight:600">✓ No active alerts</span>
            <span style="color:{TEXT_MUTED};font-size:0.82rem"> — All monitored parameters within normal range</span>
        </div>""",
        unsafe_allow_html=True,
    )
...
st.markdown("---")
st.markdown(
    f"""<div style="text-align:center;color:{TEXT_MUTED};font-size:0.72rem;padding:0.5rem 0">
        {PRODUCT_NAME} v1.0.0 · AI-Based Sustainability Monitoring System for Data Centres · MSc Dissertation Project<br>
        All values derive from synthetic operational records. Recommendations require qualified engineering review.
    </div>""",
    unsafe_allow_html=True,
)
